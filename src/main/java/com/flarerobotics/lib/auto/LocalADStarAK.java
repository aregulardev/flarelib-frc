...
public class LocalADStarAK implements Pathfinder {
    ...
    public PathPlannerPath getCurrentPath(PathConstraints constraints, GoalEndState goalEndState) {
        if (!Logger.hasReplaySource()) {
            m_io.updateCurrentPathPoints(constraints, goalEndState);
        }

        Logger.processInputs("LocalADStarAK", m_io);

        if (m_io.currentPathPoints.isEmpty()) {
            return null;
        }
    ...
    public void setStartPosition(Translation2d startPosition) {
        if (!Logger.hasReplaySource()) {
            m_io.adStar.setStartPosition(startPosition);
        }
    }
    ...
    public void setGoalPosition(Translation2d goalPosition) {
        if (!Logger.hasReplaySource()) {
            m_io.adStar.setGoalPosition(goalPosition);
        }
    }
    ...
    public void setDynamicObstacles(List<Pair<Translation2d, Translation2d>> obs, Translation2d currentRobotPos) {
        if (!Logger.hasReplaySource()) {
            m_io.adStar.setDynamicObstacles(obs, currentRobotPos);
        }
    }
