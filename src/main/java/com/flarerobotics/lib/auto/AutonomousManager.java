 ...
 *
 * <p>
 * Call {@link AutonomousManager#updateSelectedCommand()} to update the
 * retrieved command manually, as it's only updated automatically when the
...
public class AutonomousManager {
    ...
    // Config Variables
    /**
     * Whether to throw an error or to warn via DS. Generally should only be used
     * when testing and not in competition. Defaults to false.
     */
    ...
    private static final String kWidgetName = "Auto Pattern Builder"; // DO NOT CHANGE
    private static final String kWidgetDefaultTab = "Autonomous";
    private static final String kWidgetDefaultTitle = "Autonomous Routine Builder";
     ...
     * @param commands            The map of command names to commands.
     * @param constraints         The path constraints to use for pathfinding.
     * @param autoScheduleCommand Whether to automatically schedule the command when
     *                            the autonomous period begins.
     */
    public AutonomousManager(
            Map<String, Pose2d> poses,
     ...
     *
     * <p>
     * Note that {@link AutoBuilder} needs to be configured
     * if warmupPathfind is set to true.
     *
     * @param poses               The map of pose names to poses.
     * @param commands            The map of command names to commands.
     * @param constraints         The path constraints to use for pathfinding.
     * @param autoScheduleCommand Whether to automatically schedule the command when
     *                            the autonomous period begins.
     * @param warmupPathfind      Whether to call the PathPlanner pathfind warmup
     *                            command.
     *
     * @throws RuntimeException If warmupPathfind is true while AutoBuilder is not
     *                          configured, and {@link #kErrorInsteadOfWarn} is set
     *                          to true.
     */
    public AutonomousManager(
            Map<String, Pose2d> poses,
    ...
    /**
     * Generates an autonomous routine from the given string pattern. Uses the poses
     * and commands provided in the constructor. Keep in mind that the
     * {@link AutoBuilder} needs to be configured if any poses are used. Command
     * names are case-sensitive. If duplicate command-pose names are used, the
    ...
    public Command generateAutoFromString(String pathPattern) {
        ...
        for (String sequentialSection : pathPattern.split("/")) {
            ...
            if (!parallel.isEmpty()) {
                sequenced.add(
                        parallel.size() == 1 // A small optimization to not add a singular command into a
                                // parallel command group
                                ? parallel.get(0)
                                : Commands.parallel(parallel.toArray(Command[]::new)));
            }
        }
    ...
    /**
     * Initializes the command autonomous sendable chooser with the given default
     * option.
     *
     ...
     * @return The chooser.
     */
    public SendableChooser<Command> getCommandAutoChooser() {
        return m_commandChooser;
    }
    ...
    public void initFlareAutoBuilderWidget(String tab, String title) {
        Shuffleboard.getTab(tab)
                .add(title, "") // Initialize with empty string
                .withWidget(kWidgetName); // Must match @Description name
    }
    ...
    /**
     * Returns the autonomous command based on the primary source. <b>May return
     * null if a faulty supplier is passed or an error is thrown. Does not account
     * for the fallback command.</b>
     *
        ...
        try {
            ...
            switch (m_primaryCommandSource) {
                ...
                case WIDGET_AUTO_BUILDER_INTEGRATION:
                    ...
                    if (!entry.exists()) {
                        warnOrThrow("widget entry not found");
                        return null;
                    }
            ...
            if (cmd != null && m_isPreCommandSet && m_preAutoCommand != null) {
                ...
                cmd = m_isParallelPre
                        ? Commands.parallel(m_preAutoCommand, cmd)
    ...
    /**
     * Returns the retrieved auto command. Defaults to <code>Commands.none()</code>,
     * and is <b>only updated when the autonomous period begins, or when
     * {@link #updateSelectedCommand()} is called.</b>
     *
     * @return The command.
     */
    public Command getRetrievedCommand() {
        return m_mainRetrievedCommand;
    }

    // Dashboard String Input //

    /**
     * Sets the key to use for the dashboard input. This is only used when the
     * primary input mode is set to
     * {@link PrimaryCommandSource#DASHBOARD_PATTERN_STRING_INPUT}.
     *
     * <p>
    ...
    public String getDashboardPatternInputKey() {
        return m_dashboardPatternInputKey;
    }
    ...
    /**
     * Sets the fallback autonomous command for when the main autonomous command
     * fails. This command will only be used if the <code>autoScheduleCommand</code>
     * parameter is set to true, unless called manually.
     *
     ...
     * @param isParallel Whether to run the command in parallel with the autonomous
     *                   routine.
     * @return The singleton instance for chaining.
     */
     ...
     * @return The strategy.
     */
    public PathfindStrategy getPathfindStrategy() {
        return m_pathfindStrategy;
    }
     ...
     * @throws RuntimeException If an error occurs while retrieving the command and
     *                          if {@link #kErrorInsteadOfWarn} is set to true.
     */
    public Command getPathfindCommand(String poseName) {
        Pose2d pose = m_poseMap.get(poseName);
        return getPathfindCommandPrivate(pose, poseName);
    }
    ...
    public AutonomousManager initializePoseTriggers(List<ZoneTrigger> triggers) {
        if (m_zoneTriggers != null) {
            warnOrThrow("pose triggers already initialized, skipping");
            return m_instance;
        }
    ...
    /**
     * Returns the zone trigger for the given pose name. The pose name must be
     * registered in the pose map, and the triggers must be initialized via
     * {@link #initializePoseTriggers(List)}.
     *
    ...
    public Trigger getZoneTrigger(String poseName) {
        ...
        if (poseName == null || poseName.isBlank()) {
            warnOrThrow("null or empty pose name passed");
            return null;
        }

        Trigger trigger = m_zoneTriggers.get(poseName);
        if (trigger == null) {
            warnOrThrow("no trigger found for pose '" + poseName + "'");
        }
        return trigger;
    }
    ...
    public AutonomousManager setRobotPoseSupplier(Supplier<Pose2d> poseSupplier) {
        if (poseSupplier == null) {
            warnOrThrow("null pose supplier passed");
            return m_instance;
        }
    ...
    private void scheduleAuto() {
        ...
        if (m_mainRetrievedCommand != null) {
            m_mainRetrievedCommand.schedule();
            ...
            if (m_fallbackCommand != null) {
                m_fallbackCommand.schedule();
                DriverStation.reportWarning(
                        "Starting fallback autonomous routine: " + m_fallbackCommand.getName(), false);
            } else {
    ...
    /**
     * Creates a trigger zone to execute a command when the robot is close enough to
     * a pose by the given amount.
     */
