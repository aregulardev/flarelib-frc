...
public class AlignToTagCommand2D extends Command {
    ...
    public void execute() {
        ...
        ChassisSpeeds speeds = new ChassisSpeeds(
                m_xPID.calculate(botPose.getX()),
                m_yPID.calculate(botPose.getY()),
     ...
     * If no validTagIds are passed, all tags are considered.
     *
    ...
    public static int getClosestTagId(Pose2d robotPose, int... validTagIds) {
        ...
        Set<Integer> filter = (validTagIds == null || validTagIds.length == 0)
                ? null
                : Arrays.stream(validTagIds).boxed().collect(Collectors.toSet());

        Optional<AprilTag> best = VisionSubsystem.kLayout.getTags().stream()
