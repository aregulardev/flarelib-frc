...
/**
 * Simulates a continuous elevator and calculates stage positions, publishing
...
public class ContinuousAscopeDisplay {
    ...
    /**
     * The pose offset applied to the supplied pose. Useful for situations where a
     ...
     * @param totalStages     The amount of stages present. The first stage is the
     *                        carriage.
     * @param maxHeightMeters The maximum reachable height, in meters.
     * @param offset          The pose offset applied to the supplied pose, in
     *                        meters. Used when a mechanism pose is provided instead
     *                        of a carriage pose.
     * @param heightSupplier  The elevator height supplier.
     * @param angle           The elevator angle supplier.
     */
    public ContinuousAscopeDisplay(
            String name,
            int totalStages,
            double maxHeightMeters,
            double offset,
            Supplier<Double> heightSupplier,
            Supplier<Rotation3d> angle) {
        if (totalStages < 1) {
            throw new IllegalArgumentException("At least 1 stage required");
        }
        m_name = name;
        ...
        for (int i = 0; i < totalStages; i++) {
            m_stageMaxHeights[i] = maxHeightMeters / totalStages;
        }
     ...
     * @param heightSupplier  The elevator height supplier.
     * @param angle           The elevator angle supplier.
     * @param stageLengths    The length of each stage in ascending order, starting
     *                        from stage 1.
     */
    public ContinuousAscopeDisplay(
            String name,
    ...
    public void update() {
        for (int stage = 1; stage <= m_totalStages; stage++) {
            Logger.recordOutput(
                    "Simulation/" + m_name + "Elevator/Stage" + stage + "/Pose",
                    getStagePose(stage));
        }
    }
    ...
    private Pose3d createPose(double zHeight) {
        return new Pose3d().transformBy(
                new Transform3d(
                        new Translation3d(0, 0, zHeight).rotateBy(m_angle.get()),
                        m_angle.get()));
    }
