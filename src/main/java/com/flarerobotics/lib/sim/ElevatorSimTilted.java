...
//
// NOTE:
// Taken from the WPILib Java library. All credit goes to them.
//
...
public class ElevatorSimTilted extends LinearSystemSim<N2, N1, N2> {
	 ...
	 *
	 * @param plant                The linear system that represents the elevator. This system can
	 *                             be created with
	 ...
	 *                             no noise is desired. If present must have 1 element for
	 *                             position.
	 */
	@SuppressWarnings("this-escape")
	public ElevatorSimTilted(LinearSystem<N2, N1, N2> plant, DCMotor gearbox, double minHeightMeters,
			double maxHeightMeters, boolean simulateGravity, double startingHeightMeters,
	 ...
	 *                             position.
	 */
	public ElevatorSimTilted(double kV, double kA, DCMotor gearbox, double minHeightMeters, double maxHeightMeters,
			boolean simulateGravity, double startingHeightMeters, double... measurementStdDevs) {
		this(LinearSystemId.identifyPositionSystem(kV, kA), gearbox, minHeightMeters, maxHeightMeters, simulateGravity,
				startingHeightMeters, measurementStdDevs);
	}
	 ...
	 *
	 * @param gearbox              The type of and number of motors in the elevator gearbox.
	 * @param gearing              The gearing of the elevator (numbers greater than 1 represent
	 *                             reductions).
	 * @param carriageMassKg       The mass of the elevator carriage.
	 * @param drumRadiusMeters     The radius of the drum that the elevator spool is wrapped
	 *                             around.
	 * @param minHeightMeters      The min allowable height of the elevator.
	 * @param maxHeightMeters      The max allowable height of the elevator.
	 * @param simulateGravity      Whether gravity should be simulated or not.
	 ...
	 *                             position.
	 */
	public ElevatorSimTilted(DCMotor gearbox, double gearing, double carriageMassKg, double drumRadiusMeters,
			double minHeightMeters, double maxHeightMeters, boolean simulateGravity, double startingHeightMeters,
			double... measurementStdDevs) {
		this(LinearSystemId.createElevatorSystem(gearbox, carriageMassKg, drumRadiusMeters, gearing), gearbox,
				minHeightMeters, maxHeightMeters, simulateGravity, startingHeightMeters, measurementStdDevs);
	}
	 ...
	 */
	public double getPositionMeters() { return getOutput(0); }
	 ...
	 */
	public double getVelocityMetersPerSecond() { return getOutput(1); }
	...
	public void setInputVoltage(double volts) {
		setInput(volts);
		if (Robot.isSimulation()) clampInput(RoboRioSim.getVInVoltage());
		else clampInput(RobotController.getBatteryVoltage());
	}
	...
	protected Matrix<N2, N1> updateX(Matrix<N2, N1> currentXhat, Matrix<N1, N1> u, double dtSeconds) {
		...
		var updatedXhat = NumericalIntegration.rkdp((x, _u) -> {
			...
			if (m_simulateGravity) { xdot = xdot.plus(VecBuilder.fill(0, -9.8 * Math.abs(Math.cos(m_tilt)))); }
			return xdot;
