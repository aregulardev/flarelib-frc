...
import com.flarerobotics.lib.subsystem.objectDetection.ObjectDetectionIO.GamePieceObservation;
...
public class ObjectDetectionSubsystem extends SubsystemBase {
	...
	public ObjectDetectionSubsystem(String gamePieceName, ObjectDetectionIO... cameras) {
		...
		for (int i = 0; i < m_inputs.length; i++) {
			m_disconnectedAlerts[i] = new Alert(
					"Detection camera with index '" + Integer.toString(i) + "' is disconnected.", AlertType.kWarning);
		}
	}
	...
	public void periodic() {
		...
		for (int i = 0; i < m_cameras.length; i++) {
			...
			m_disconnectedAlerts[i].set(!input.isCameraConnected);
