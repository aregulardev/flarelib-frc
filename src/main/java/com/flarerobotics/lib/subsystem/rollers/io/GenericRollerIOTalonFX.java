package com.flarerobotics.lib.subsystem.rollers.io;
...
import com.flarerobotics.lib.subsystem.rollers.GenericRollerIO;
...
public class GenericRollerIOTalonFX implements GenericRollerIO {
	...
	public GenericRollerIOTalonFX(int mainId, int followerId, boolean mainInverted, boolean followerInverted,
		...
		m_talonMain = new TalonFX(mainId, mainCan != null ? mainCan : "");
		if (followerId != -1) m_talonFollower = new TalonFX(followerId, followerCan != null ? followerCan : "");
		...
		if (followerId != -1) {
			...
			retryUntilOk(() -> m_talonFollower.setControl(new Follower(mainId, mainInverted != followerInverted)),
		...
		BaseStatusSignal.setUpdateFrequencyForAll(50, m_volts, m_position, m_velocity, m_supplyCurrent, m_statorCurrent,
				m_temperature);
