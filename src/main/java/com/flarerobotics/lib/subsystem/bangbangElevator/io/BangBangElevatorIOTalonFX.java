package com.flarerobotics.lib.subsystem.bangbangElevator.io;
...
import com.flarerobotics.lib.subsystem.bangbangElevator.BangBangElevatorIO;
...
public class BangBangElevatorIOTalonFX implements BangBangElevatorIO {
	...
	private VoltageOut m_voltageControl = new VoltageOut(0).withUpdateFreqHz(0);
	private TorqueCurrentFOC m_currentControl = new TorqueCurrentFOC(0).withUpdateFreqHz(0);
	...
	public BangBangElevatorIOTalonFX(int mainId, int followerId, boolean mainInverted, boolean followerInverted,
		...
		m_mainTalon = new TalonFX(mainId, mainCan != null ? mainCan : "");
		if (followerId != -1) m_followerTalon = new TalonFX(followerId, followerCan != null ? followerCan : "");
		...
		if (followerId != -1) {
			...
			retryUntilOk(() -> m_followerTalon.setControl(new Follower(mainId, mainInverted != followerInverted)),
		...
		BaseStatusSignal.setUpdateFrequencyForAll(50, m_velocity, m_position, m_volts, m_supplyCurrent, m_statorCurrent,
				m_temp);
